'''
Similar system to 'solar_system.py' but with the addition of Haleys Comet.
'''
...
class Halley(System):
    ...
    def _create_planets(self):
        ...
        # Add each planet starting at its perihelion (closest approach to Sun)
        for name, mass, a, e in planet_data:
            # Distance at perihelion
            r_peri = a * (1 - e)

            # Velocity at perihelion
            # For an elliptical orbit: v_peri = sqrt(μ*(1+e)/(a*(1-e)))
            # where μ = G*M_sun = G*1 in our units
        ...
        for i in range(n_bodies):
            ...
            for j in range(n_bodies):
                if i != j:  # Skip self-interaction
                    ...
                    # a = G*m_j*r_vec/r³
                    acceleration += self.G * mass_j * r_vec / (r_mag ** 3)
